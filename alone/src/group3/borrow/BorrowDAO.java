...
public class BorrowDAO {
    ...
    public int borrBook(String memId, int bookNum) {
		return template.update("INSERT INTO BORR(MEM_ID, BOOK_NUM, BORR_EXTE_CNT, BORR_END_DATE, BORR_DATE, BORR_OVE_DATE) VALUES(?, ? ,1 ,sysdate + 5, sysdate, -5)", memId, bookNum);
    }
    public int borrBook2(String memId, int bookNum) {
		return template.update("INSERT INTO BORR(MEM_ID, BOOK_NUM, BORR_EXTE_CNT, BORR_END_DATE, BORR_DATE, BORR_OVE_DATE) VALUES(?, ? ,3 ,sysdate + 10, sysdate, -10)", memId, bookNum);
    }
    ...
    public int extendBook(int extend) {
        return template.update("UPDATE BORR SET BORR_END_DATE = BORR_END_DATE +7, BORR_EXTE_CNT = BORR_EXTE_CNT -1 , BORR_OVE_DATE = TO_DATE(SYSDATE) - TO_DATE(BORR_END_DATE) "
              + "WHERE BOOK_NUM = ? AND BORR_EXTE_CNT > 0 and BORR_OVE_DATE < 0 ", extend);
    }
	//연체
